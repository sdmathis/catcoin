...
{
    // return if URI is not valid or is no catcoin URI
...
{
    // Convert catcoin:// to catcoin:
    //
    //    Cannot handle this later, because catcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("catcoin://"))
    {
        uri.replace(0, 10, "catcoin:");
    }
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a catcoin.desktop file to the autostart directory:
